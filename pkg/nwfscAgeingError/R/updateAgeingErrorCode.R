...
updateAgeingErrorCode <- function (local = NULL, save = FALSE, revision = "newest",env="nwfscAgeingError",pos=2)
{
    ...
    getwebnames <- function() {
        changes <- readLines("http://r-forge.r-project.org/scm/viewvc.php/pkg/nwfscAgeingError/R/?root=nwfscassmt")
        ...
        if (revision == "newest") {
            webdir <- "http://r-forge.r-project.org/scm/viewvc.php/pkg/nwfscAgeingError/R/?root=nwfscassmt"
    ...
    getwebfiles <- function(fileinfo) {
        ...
        webdir <- "http://r-forge.r-project.org/scm/viewvc.php/*checkout*/pkg/nwfscAgeingError/R/"   #GetAges.fn.R?revision=7&root=nwfscassmt
        n <- length(filenames)
        cat(n, "files found\n")
        if (save) {
