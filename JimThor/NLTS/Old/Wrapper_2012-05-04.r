...
NltsPred(Y, PredInterval=1, Nembed=4, PredNum=length(Y)+1, Method="Simplex")  # PredInterval is the number of years in the future (1 is next year, etc)

# S-map
Output = NltsFn(Y, Nembed=4, Method="Smap", Theta=1)
ThetaFn(Y, Nembed=4)
NltsPred(Y, PredInterval=1, Nembed=4, PredNum=length(Y)+1, Method="Smap", Theta=1)
